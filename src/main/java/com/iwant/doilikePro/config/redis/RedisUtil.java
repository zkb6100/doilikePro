...
public class RedisUtil<K, V> {
	...
	public boolean expire(K key, long time) {
		try {
			...
			logger.error("expire 指定缓存失效时间 error", e);
			return false;
		}
	}
	...
	public boolean hashKey(K key) {
		try {
			...
			logger.error("判断key 是否存在 redis hashKey error", e);
			return false;
		}
	}
	/**
 ...
 // ============================String=============================
	...
	public boolean set(K key, V value) {
		try {
			//最好判断一下 key  和 value 放入空值 没必要
			if(key != null && value != null) {
				redisTemplate.opsForValue().set(key, value);
				return true;
			}		
		} catch (Exception e) {
			logger.error("普通缓存放入 redis set error", e);
		}
		return false;
	}
	...
	public boolean set(K key, V value, long time) {
		try {
			...
			logger.error("普通缓存放入 设置过期时间 redis set error", e);
		}
		return false;
	}
     ...
     * @param keyAndValue 里面有判断 map  所以这里可以不用判断
     */
